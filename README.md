...
**1초 간격 저장** 체크박스를 켜면 다운샘플링 없이 fit의 record를 전부 담는다. 10~20초짜리
질주처럼 짧은 구간은 10초 간격으로는 점이 한두 개밖에 안 남아 분석이 안 되기 때문이다.
대신 파일이 10배 가까이 커지고(23분 러닝 실측 82KB → 711KB), 파일명 끝에 `_1s`가 붙어
10초 파일과 나란히 남는다.
...
- **FIT 파싱** — fit은 바이너리 포맷이라 직접 뜯지 않고 **Garmin 공식 FIT SDK**로
  ...
  **1초 간격 저장을 켜면 다운샘플링을 건너뛰고 record를 전부 담는다.** 실제로 쓴 간격은
  `source.recordSamplingIntervalSec`에 기록되므로 파일만 봐도 어느 쪽인지 알 수 있다.
...
훑으면서 정해진 간격(기본 10초, 1초 간격 저장을 켜면 1초)만 남기고 버린다.
...
| `lap`(19) | 랩별 요약 |
| `record`(20) | 초 단위 기록 → 10초 간격으로 다운샘플해 `records[]`에 담는다(1초 간격 저장을 켜면 전부) |
