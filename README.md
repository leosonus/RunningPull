...
초 단위 원시 기록(`records[]`)과 날씨(기온·습도·이슬점·체감온도·풍속·풍향)도 함께 담아서
...
- **FIT 파싱** — Garmin FIT SDK로 세션/랩 요약뿐 아니라 초 단위 record 전체(`records[]`)까지
  뽑는다. 심박 드리프트, 오르막 영향, 케이던스 변화, 질주 구간처럼 요약만으로는 안 보이는
  것까지 분석할 수 있게 하려는 것.
